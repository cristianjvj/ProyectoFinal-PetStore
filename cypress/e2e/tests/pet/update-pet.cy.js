...
describe('Update pet', () => {
    it('Update pet - happy path', () => {
        ...
        const updatedBody = new PetBodyBuilder().setPetId(petId).setCategoryId(categoryId).setCategory(category).setPetName(petName).setPhotoUrls(photoUrls).setTags([tags]).setStatus(status).build()
